...
static const char *DEFAULT_X265_PARAMS =
    "keyint=9999:min-keyint=9999:scenecut=0:bframes=0:"
    "ref=1:rc-lookahead=0";

int mv_init(int w, int h, int fps, const char* x265_params)
{
    const AVCodec *enc_c = avcodec_find_encoder_by_name("libx265");
    const AVCodec *dec_c = avcodec_find_decoder(AV_CODEC_ID_HEVC);
    ...
    av_opt_set(enc->priv_data,"profile","main",0);
    av_opt_set(enc->priv_data,"tune","zerolatency",0);

    if (x265_params && strlen(x265_params) > 0)
        av_opt_set(enc->priv_data,"x265-params", x265_params, 0);
    else
        av_opt_set(enc->priv_data,"x265-params", DEFAULT_X265_PARAMS, 0);
                          ...
                          int *encoded_size_out, int *mv_cnt_out)
{
    ...
    while(avcodec_receive_packet(enc,pkt)==0){
        if(encoded_size + pkt->size <= out_buf_size) {
            memcpy(out_buf + encoded_size, pkt->data, pkt->size);
